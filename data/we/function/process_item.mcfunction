...
$execute if items block ~ ~-1 ~ container.0 $(item) if scoreboard players get $(id) we_items_forged = 0 run tellraw @a [{"text":"[World Forge] ","color":"aqua","bold":true},{"text":"A new item has been forged for the first time!","color":"green","italic":true}]
