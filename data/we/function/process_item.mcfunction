...
$execute if items block ~ ~-1 ~ container.0 $(item) run say Adding to scoreboard $(scoreboard)
# $execute if items block ~ ~-1 ~ container.0 $(item) run scoreboard players set $(id) $(scoreboard) = scoreboard players get $(id) we_items_forged
