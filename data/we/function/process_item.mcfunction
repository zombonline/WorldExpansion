...
$execute if items block ~ ~-1 ~ container.0 $(item) run tellraw @a {"text":"Detecting $(id)"}
...
$execute if items block ~ ~-1 ~ container.0 $(item) if score $(id) we_items_forged matches 1 run function we:world_forge_announcement {message:"A new item has been forged for the first time: $(display)!","color":"green"}
