...
$execute if items block ~ ~-1 ~ container.0 $(item) if score $(id) we_items_forged matches 1 run tellraw @a [{"text":"[World Forge] ","color":"aqua","bold":true},{"text":"A new item has been forged for the first time!","color":"green","italic":true}]
